...
async def main():
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        for case in CASES:
            ...
            for (name, _url), (elapsed, data, error) in zip(TARGETS.items(), fetched):
                ...
                for src in sources[:5]:
                    if isinstance(src, dict):
                        top_sources.append(f"{src.get('sourcefile', '?')}|{src.get('subsection_id', '?')}")
            ...
            results.append(row)
    output_path = pathlib.Path("/tmp/local_vs_deployed_quality_small.json")
