...
export function fixMalformedCitations(text: string): string {
    ...
    // The LLM sometimes outputs citations as bare numbers without brackets:
    //   "...occurs. 1 The Protocol..." → "...occurs.[1] The Protocol..."
    //   "...stage. 2 It provides..." → "...stage.[2] It provides..."
    //   "...party. 1" (end of text) → "...party.[1]"
    // Also handles chained bare citations after a previously fixed bracket:
    //   "...issues.[4]  1 Pre-action..." → "...issues.[4][1] Pre-action..."
    // Matches after sentence-ending punctuation (. ! ? ]) followed by 1-2 digit number,
    // then either a new sentence (uppercase letter), another bare number, or end of text.
    // Runs iteratively to handle chained bare citations (e.g., ". 4 1 Pre-action").
    const bareCitationPattern = /([.!?\]])\s+(\d{1,2})(?=\s+[A-Z]|\s+\d{1,2}(?:\s|$)|\s*$)/gm;
    let prev = "";
    while (prev !== result) {
        prev = result;
 ...
 * These are decimal-numbered references like [7.3], [9.1] that the LLM inserts
 * alongside actual source citations. They confuse the citation parser because they
 * appear in bracket format but are CPR paragraph references, not source indices.
 *
 * Examples:
 * - "[7.3]" → "" (CPR paragraph reference removed)
 * - "[9.1]" → "" (CPR section reference removed)
 * - "[7.3(2)]" → "" (CPR sub-paragraph reference removed)
 * - "[7.3(a)]" → "" (CPR lettered sub-paragraph removed)
 */
export function removeDecimalBracketReferences(text: string): string {
    // Match [N.M] or [N.M(N)] or [N.M(a)] patterns - these are CPR section references, not source citations
...
export function sanitizeCitations(text: string): string {
    ...
    // Must run before fixMalformedCitations so chained patterns like "4 [7.3] 1" become "4  1"
    result = removeDecimalBracketReferences(result);

    // First fix malformed unbracketed citations like "1. 1" → "[1]" and bare ". 1 The" → ".[1] The"
