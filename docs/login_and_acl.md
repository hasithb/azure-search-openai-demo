...
- `AZURE_ENABLE_UNAUTHENTICATED_ACCESS`: Allows unauthenticated users to access the chat app backend. If `AZURE_ENFORCE_ACCESS_CONTROL` is enabled, unauthenticated users cannot search on documents.
- `AZURE_ENABLE_HOST_UNAUTHENTICATED_ACCESS`: Allows the hosting platform (App Service or Container Apps auth) to serve the SPA shell without login while keeping backend chat APIs authenticated when `AZURE_ENABLE_UNAUTHENTICATED_ACCESS` remains `false`. Defaults to the value of `AZURE_ENABLE_UNAUTHENTICATED_ACCESS` for backwards compatibility.
